...
/*   Updated: 2021/06/24 10:38:06 by jsiller          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*ft_precise_b(char *str, char *ret, int i, int k)
{
	...
	while (k < g_precision - ft_strlen(str))
	{
		ret[i] = '0';
		i++;
		k++;
...
static char	*ft_precise(char *str)
{
	...
	if (g_precision < ft_strlen(str))
		ret = malloc(ft_strlen(str) + g_width + 1);
	else
		ret = malloc(g_precision + 1);
	if (!ret)
		return (0);
	if (g_padding && g_width > ft_strlen(str) && g_precision == -1)
	{
		g_precision = g_width;
		if (str[0] == '-')
			g_precision--;
	}
	if (ft_strlen(str) == 1 && str[0] == '0' && g_precision == 0)
	{
		ret[0] = 0;
		return (ret);
	}
